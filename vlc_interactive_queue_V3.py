# vlc_interactive_queue_V3.py

import os
import sys

# ─────────────────────────────────────────────────────────────────────────────
#  0) BEFORE importing 'vlc', force VLC’s own folders onto PATH and set VLC_PLUGIN_PATH
#     ↳ On Windows, if python‐vlc can’t find 'libvlc.dll' or the 'plugins' folder early,
#       then media_list_new() will return None. We must set these *before* import vlc.
# ─────────────────────────────────────────────────────────────────────────────

vlc_base    = r"C:\Program Files\VideoLAN\VLC"
vlc_plugins = os.path.join(vlc_base, "plugins")

# 0A) Add VLC root folder (where libvlc.dll lives) to the OS PATH
path_env = os.environ.get("PATH", "")
if vlc_base not in path_env:
    os.environ["PATH"] = vlc_base + ";" + path_env

# 0B) Explicitly tell libVLC where to find its plugin libraries
os.environ["VLC_PLUGIN_PATH"] = vlc_plugins

# Now that PATH and VLC_PLUGIN_PATH are set, python‐vlc can load properly:
import ctypes
...
class VLCInteractiveQueue:
    def __init__(self):
        ...
        # ─────────────────────────────────────────────────────────────────────
        #  4) Instantiate VLC Instance (now that PATH & VLC_PLUGIN_PATH are correct),
        #     then create the MediaList and MediaListPlayer.
        # ─────────────────────────────────────────────────────────────────────
        self.instance   = vlc.Instance()
        self.media_list = self.instance.media_list_new()
        if not self.media_list:
            print("ERROR: instance.media_list_new() returned None.")
            print("       Double‐check that VLC_PLUGIN_PATH and PATH are set correctly,")
            print("       and that they point to a valid VLC installation with a 'plugins' folder.")
            self.root.destroy()
            sys.exit(1)
        ...
        # Embed VLC’s video output into our Tk frame
        ...
        # ─────────────────────────────────────────────────────────────────────
        #  7) Start polling playback state and enter Tk mainloop
        # ─────────────────────────────────────────────────────────────────────
        self._poll_playback()
        self.root.mainloop()
        self.ml_player.stop()
    ...
    def _poll_playback(self):
        """
        Every 500 ms, check if VLC is idle (Ended/Stopped) but the queue still
