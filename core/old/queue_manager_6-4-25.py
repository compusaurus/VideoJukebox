...
class QueueManager:
    ...
    def remove_song_from_app_view(self, song_info_to_remove): # If admin removes
        # This is more complex now as we'd need to remove from VLC's MediaList too.
        # For now, this method would just remove from our app_queue_view.
        # Removing from VLC's MediaList by item needs care (by index or by Media object).
        try:
            self.app_queue_view.remove(song_info_to_remove) # If song_info are dicts and comparable
            logger.info(f"Removed {song_info_to_remove['title']} from app queue view.")
            return True
        except ValueError:
            logger.warning(f"Could not remove {song_info_to_remove['title']} from app queue view (not found).")
            return False
