...
class MainUI:
    ...
    def load_initial_results(self):
        """Populates the results tree with all videos or a default view."""
        self.app.logger.info("MainUI.load_initial_results - Called.")
        self.search_entry_var.set("") # Ensure query is empty for perform_search
        self.perform_search() # This will now get all videos
    ...
    def add_selected_to_queue(self):
        if self.current_selected_song_details:
            ...
            # QueueManager handles all credit logic now
            success, message = self.app.queue_manager.add_song_to_system(song_to_add, self.app.video_player)
            
            if success:
                self.app.logger.info(f"Song '{song_to_add['title']}' processed by QueueManager.")
                messagebox.showinfo("Queue Update", message, parent=self.window)
                self.app.update_all_ui_elements()
                
                player_state = self.app.video_player.get_state() # Assuming get_state is the correct method name
                if player_state not in [vlc.State.Playing, vlc.State.Opening, vlc.State.Buffering]:
                    self.app.video_player.play_playlist() 
                
                self.show_search_view()
            else:
                # If adding failed, QueueManager already handled any necessary refund.
                # Just show the error message from QueueManager.
                messagebox.showerror("Queue Error", message, parent=self.window)
                # DO NOT try to refund credits here again.
        else:
...
# ui/main_ui.py
    ...
    def update_queue_display(self):
        ...
        if self.app and hasattr(self.app, 'queue_manager') and self.app.queue_manager: # Add safety check
            # Use the correct method name here
            items_to_display = self.app.queue_manager.get_app_queue_view_strings(limit=10) 
            for item_text in items_to_display:
                self.queue_listbox.insert(tk.END, item_text)
        else:
            self.app.logger.warning("update_queue_display: queue_manager not available on app object.")
